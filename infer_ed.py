...
def run():
    ...
    for index, category in enumerate(tqdm(category_list)):
        # if not index < 15:
        #     continue
        save_path = f"./output/ace_plus_edit/{category}"
        ...
        for img_name in all_bg[index::category_num]:
            ...
            if os.path.exists(f"./{save_path}/{img_name}+{prompt}+{seed}.jpg"):
                print(f"Already exist:  ./{save_path}/{img_name}+{prompt}+{seed}.jpg")
                continue
            # print(cfg.args.task_type.upper(), task_model_cfg)
            # assert cfg.args.task_type.upper() in task_model_cfg
            params = {
                "input_image": f"{tar_path}/{img_name}_1.png",
                "input_mask": f"{tar_path}/{img_name}_2.png",
                "input_reference_image": f"{ref_path}/{category}/image/00.jpg",
                "save_path": f"./{save_path}/{img_name}+{prompt}+{seed}.jpg",
